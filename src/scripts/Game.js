...
class Game {
  ...
  initialize() {
    ...
    for (let i = 0; i < this.towerTypes.length; i++) {
      document
        .querySelector(`.${this.towerTypes[i].type}`)
        .addEventListener("click", () => {
          if (
            this.resources >= this.towerTypes[i].cost &&
            Object.keys(this.battlefield.towers).length < 6
          ) {
            this.resources -= this.towerTypes[i].cost;
    ...
    document
      .querySelector(".upgrade-income")
      .addEventListener("click", this.upgradeIncomeBind);
  ...
  intro() {
    ...
    document
      .querySelector(".hide-instructions-button")
      .addEventListener("click", this.hideInstructions);
  ...
  startGame() {
    this.resourceInterval = setInterval(
      this.generateResources.bind(this),
      10 / this.resourceRate
    );
  ...
  lose() {
    ...
    document
      .querySelector(".upgrade-income")
      .removeEventListener("click", this.upgradeIncomeBind);
  }
