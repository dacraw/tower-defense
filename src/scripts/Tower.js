...
class Tower {
  ...
  levelUp() {
    ...
    const tower = document.querySelector(
      `.tower-${this.id} .tower-level strong`
    );
  ...
  attack(enemy, game) {
    ...
    const towerDamageEle = document.querySelector(
      `.tower-modal-wrapper.tower-${this.id} .tower-damage strong`
    );
    const damageMultiplier = parseFloat(
      this.damage * ((this.towerLevel - 1) / 10 + 1).toFixed(2)
    );
