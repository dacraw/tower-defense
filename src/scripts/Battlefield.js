...
class Battlefield {
  constructor(game) {
    ...
    this.castleCoords = [
      ...
      this.canvas.height * 0.07,
    ];
    this.firstTowerCoords = [
      this.castleCoords[0] + this.canvas.width * 0.02,
      this.castleCoords[1] + this.castleCoords[3] * 0.7,
      this.canvas.width * 0.133,
  ...
  findClosestEnemies() {
    ...
    const availableEnemies = enemiesArr.filter(
      (enemy) => enemy.coords[1] > 0 && enemy.currentHealth > 0
    );
    const availableCoords = availableEnemies.map((enemy) => enemy.coords[1]);
    const closestThreeCoords = availableCoords
      .sort((a, b) => b - a)
      .slice(0, 3);
    const closestEnemies = availableEnemies
      .filter((enemy) => closestThreeCoords.includes(enemy.coords[1]))
      .sort((a, b) => b - a);
  ...
  attackEnemies(game) {
    ...
    towerIds.forEach((towerId) => {
      ...
      const closestCoord = Math.max(
        ...closestEnemies.map((enemy) => enemy.coords[1])
      );
      const closestEnemy = closestEnemies.filter(
        (enemy) => enemy.coords[1] == closestCoord
      )[0];
  ...
  createTower({ type }) {
    ...
    if (!this.nextTower.length) {
      ...
      let x = next[0] + this.firstTowerCoords[2] + this.canvas.width * 0.02;
      let y = next[1];
      let width = next[2];
      let height = next[3];
      let nextId = id + 1;
    ...
    document
      ...
      .addEventListener("click", this.showTowerModal(tower).bind(this));
  }
  refreshTowerModal(tower) {
    const level = document.querySelector(
      `.tower-modal-wrapper.tower-${tower.id} .tower-level strong`
    );
    const damage = document.querySelector(
      `.tower-modal-wrapper.tower-${tower.id} .tower-damage strong`
    );
    const enemiesDefeated = document.querySelector(
      `.tower-modal-wrapper.tower-${tower.id} .enemies-defeated strong`
    );

    level.innerText = `${tower.towerLevel}`;
    const damageMultiplier =
      Math.round(tower.damage * ((tower.towerLevel - 1) / 10 + 1) * 1e2) / 1e2;
    damage.innerText = `${damageMultiplier}`;
    enemiesDefeated.innerText = `${
      tower.enemiesDefeated + (tower.towerLevel - 1) * 10
    }`;
  }
  addTowerModal(tower) {
    const modalX = [
      this.castleCoords[0],
      this.castleCoords[0] + this.castleCoords[2],
    ];
    const modalY = [
      this.firstTowerCoords[1] +
        this.firstTowerCoords[3] * 2 -
        this.canvas.height * 0.01,
      this.firstTowerCoords[3],
    ];
  ...
  sellTower(towerId) {
    ...
    const refund = (tower.cost + (tower.upgradeCost - 100)) * 0.8;
  ...
  upgradeTower(towerId) {
    ...
    if (this.game.resources >= tower.upgradeCost) {
      ...
      document.querySelector(
        `.tower-${towerId} .current-upgrade-cost`
      ).innerText = tower.upgradeCost;
    }
  }
  hideTowerModals() {
  ...
  addTowerStyleBox(tower) {
    ...
    document.querySelector(".canvas-container").append(towerModalWrapper);
    const towerBox = document.querySelector(
      `.tower-modal-wrapper.tower-${tower.id}`
    );
  ...
  createEnemies(currentLevel) {
    let enemyWidth = this.canvas.width * 0.15;
    let enemyHeight = this.canvas.height * 0.07;
    ...
    for (let i = 0; i < this.numEnemies; i++) {
      ...
      if (
        this.canvas.width - maxX >= 0 &&
        this.canvas.width - maxX <= enemyWidth
      ) {
        // > max length
        maxX -= enemyWidth;
      } else if (maxX <= enemyWidth) {
        // < max length
        maxX += enemyWidth;
      }
      ...
      let enemy = new Enemy(
        this.ctx,
        [maxX, maxY, enemyWidth, enemyHeight],
        health,
        speed,
        i
      );
